...
struct Config
{
    ...
    Config(const ros::NodeHandle &nh_priv)
    {
        ...
        nh_priv.getParam("OptBegin", optbegin);
    }
};
...
public:
    ...
    inline void plan(const nav_msgs::Path::ConstPtr &msg)
    {
        if (config.optbegin)
        {
            // 记录时间
    ...
    inline void plan()
    {
        if (startGoal.size() >= 2)
        {
            ...
            // for (const auto &pose : msg->poses)
            // {
            //     route.emplace_back(pose.pose.position.x, pose.pose.position.y, 0);
            // }
            // std::cout<<"route size: "<<route.size()<<std::endl;
            sfc_gen::planPath<voxel_map::VoxelMap>(startGoal[0],
                                                   startGoal[1],
    ...
    inline void targetCallBack(const nav_msgs::Path::ConstPtr &msg)
    {
        if (mapInitialized)
        {
            // if (startGoal.size() >= 2)
    ...
    inline void targetCallBack0(const geometry_msgs::PoseStamped ::ConstPtr &msg)
    {
        if (mapInitialized)
        {
            if (startGoal.size() >= 2)
