...
Maps::perlin3D()
{
  ...
  void Maps::generateCylinderMap() {
    double min_radius, max_radius, min_height, max_height;
    int num_cylinders, num_points_per_cylinder;
    double map_min_, map_max_;

    // 从参数服务器获取参数
    info.nh_private->param("map_min", map_min_, -5.0);
    info.nh_private->param("map_max", map_max_, 5.0);
    info.nh_private->param("min_radius", min_radius, 0.2);
    info.nh_private->param("max_radius", max_radius, 0.5);
    info.nh_private->param("min_height", min_height, 0.0);
    info.nh_private->param("max_height", max_height, 1.0);
    info.nh_private->param("num_cylinders", num_cylinders, 30);
    info.nh_private->param("num_points_per_cylinder", num_points_per_cylinder, 100);

    info.cloud->width  = num_cylinders * num_points_per_cylinder;
    info.cloud->height = 1;
    info.cloud->points.resize(info.cloud->width);

    std::random_device rd;
    std::mt19937 gen(info.seed);
    std::uniform_real_distribution<> x_dist(map_min_, map_max_);
    std::uniform_real_distribution<> y_dist(map_min_, map_max_);
    std::uniform_real_distribution<> radius_dist(min_radius, max_radius);
    std::uniform_real_distribution<> angle_dist(0, 2 * M_PI);
    std::uniform_real_distribution<> z_dist(0, 1.0);
    ...
    for (int i = 0; i < num_cylinders; ++i) {
      double center_x = x_dist(gen);
      double center_y = y_dist(gen);
      double radius = radius_dist(gen);
      double height = height_dist(gen);

      // 为当前圆柱生成点云
      for (int j = 0; j < num_points_per_cylinder; ++j) {
        double angle = angle_dist(gen);
        double z = z_dist(gen) * height;

        double x = center_x + radius * std::cos(angle);
        double y = center_y + radius * std::sin(angle);

        info.cloud->points[pos].x = x;
        info.cloud->points[pos].y = y;
        info.cloud->points[pos].z = z;
        pos++;
      }
    }
  ...
  info.cloud->width = pos;
  info.cloud->points.resize(info.cloud->width);
  ROS_INFO("Generated %d points in the cylindrical map", pos);
